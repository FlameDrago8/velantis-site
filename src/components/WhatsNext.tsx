      ...
      <div className="mx-auto max-w-6xl px-6 py-24 md:py-32">
        ...
        <div className="mt-16 border-t border-hairline">
          {directions.map((direction, i) => (
            <Reveal key={direction.title} delay={i * 60}>
              <div className="channel flex flex-col gap-2 border-b border-hairline py-7 md:flex-row md:items-baseline md:justify-between md:gap-8">
                <h3 className="channel-title font-display text-lg font-medium tracking-tight md:w-64 md:shrink-0">
                  {direction.title}
                </h3>
                <p className="text-sm leading-relaxed text-muted md:flex-1">
                  {direction.body}
                </p>
                <span className="font-mono text-[10px] tracking-[0.22em] text-xenon uppercase md:shrink-0">
                  {direction.tag}
                </span>
              </div>
            </Reveal>
